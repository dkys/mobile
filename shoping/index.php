...
if (!empty($_GET['gOo']))
{
    ...
    {
        /* 商品分类。*/
        $Loaction = 'category.html?id=' . $_GET['gcat'];
    }
    elseif (!empty($_GET['acat']))
    {
        /* 文章分类。*/
        $Loaction = 'article_cat.html?id=' . $_GET['acat'];
    }
    elseif (!empty($_GET['goodsid']))
    {
        /* 商品详情。*/
        $Loaction = 'goods.html?id=' . $_GET['goodsid'];
    }
    elseif (!empty($_GET['articleid']))
    {
        /* 文章详情。*/
        $Loaction = 'article.html?id=' . $_GET['articleid'];
    }
...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
    ...
    $smarty->assign('css_boot',     './themes/cpts_467_tq/css/bootstrap.min.css');       // css所在目录
