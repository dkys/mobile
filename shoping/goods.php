...
/*------------------------------------------------------ */
/* 获取品牌列表 */
$sql = "SELECT b.brand_id, b.brand_name FROM" . $ecs->table("brand")."as b";
$brands_info = $db->getAll($sql, true);
$brands = array();
foreach($brands_info as $v){
    
    array_push($v,"category.php?id=3&brand={$v['brand_id']}&price_min=0&price_max=0");
    array_push($brands,$v);
}
